...
const PostItem: FC<PostItemProps> = ({post}) => {
    ...
    useEffect(() => {
        ...
        const getUser = async () => {
            try {
                const res = await axios.get('https://vcid.yanik.pro/api/user/' + post.user_id + '?UserDetailsOnly=true');
                const user = {
        ...
        <div className={"w-full p-3 bg-neutral-800 rounded-lg h-auto mb-5"}>
            ...
            <div className={"flex justify-between"}>
                <div className={"w-full flex items-center "}>
                    ...
                    <div onClick={() => commentModal.onOpen(comments, post.id, setComments)} className={"underline flex cursor-pointer p-1"}>
